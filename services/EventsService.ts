...
const createTeam = async (event: Event, user: SessionUser, teamName: string) => {
...
const joinTeam = async (event: Event, user: SessionUser, teamCode: string) => {
    ...
    if(team.memberIds.length === event.maxMembers) return {ok: false, message: "Team full"};
    ...
    else return {ok: false, message: "Unknown error occurred. Please try again later"};
}

const completeRegistration = async (team: Team, event: Event) => {
    const eligible = team.members.length <= event.maxMembers && team.members.length >= event.minMembers;
...
const removeMember = async (team: Team, memberId: string) => {
    const updatedMemberIds = team.members.filter(member => member.id && member.id != memberId).map(member => member.id??"");
    await prisma.team.update({
        where: {
            id: team.id
        },
...
export {getRegistrationStatus, getEventFromSlug, createTeam, joinTeam, completeRegistration, resetJoiningCode, transferTeamLead, removeMember};